...
; Modified 2019 by SparrOSDeveloperTeam
;
...
;   Now supports /t commandline switch to tell ansi about new text modes.
...
install_msg	db	'ANSI.sys driver installed! Based on NANSI project.'
    ...
    db  9,'Modified by SparrOSDeveloperTeam for casmOS. https://samyoeyt.wixsite.com/casmos', 13, 10
...
arg_p:		; /P option (chain) takes no argument
		...
		; which ANSI cannot handle to other CONs which can.
		mov	dx, offset popt_msg
		mov	ah, 9
		int	21h
		mov	al, 1
		mov	chain_handlers, al
		; Find pointer to current CON, the one which ANSI
...
; wrong:	and	ax,0ffbfh	; ...
		...
		; the ANSI features), R=rawmode (no cr/lf/eof stuff),
...
arg_bad_msg	db "ANSI.SYS: Possible options are /S /X /K /C /R /B /Q /P /Tnn", 13, 10
		...
		db 9,"/Tnn make ANSI treat mode nn (2 hexadecimal digits) as text mode", 13, 10
		db 9,"Example: ANSI.SYS /S /B /T0b /T4f /T51 /T52", 13, 10, "$"

kkr_off_msg	db "ANSI.SYS /S: Safe mode - disabling keyboard redefinition."
		db 13, 10, "$"
xopt_msg	db "ANSI.SYS /X: Recognize keypad keys independently."
		db 13, 10, "$"
kopt_msg	db "ANSI.SYS /K: Force 84 key keyboard behaviour."
		db 13, 10, "$"
copt_msg	db "ANSI.SYS /C: Force 101+ key keyboard behaviour."
		db 13, 10, "$"
ropt_msg	db "ANSI.SYS /R: Force BIOS calls for putchar."
		db 13, 10, "$"
bopt_msg	db "ANSI.SYS /B: Force BIOS calls for beeping."
		db 13, 10, "$"
qopt_msg	db "ANSI.SYS /Q: Ignore beep / bell character."
		db 13, 10, "$"
popt_msg	db "ANSI.SYS /P: Enable chaining to other drivers."
		db 13, 10, "$"
no_con_msg	db "ANSI.SYS /P: The stdin driver to which we chained was no"
...
gh_errmsg	db	"ANSI.SYS: Expected two-digit hexadecimal number in CONFIG.SYS"
		db	", but got $"
